import asyncio
import json
from tkinter import Entry, Button, Label, W, E, N, messagebox
...
async def draw_successful_registration(account_hash, nickname, interval=1 / 120):
    ...
    nickname_field = Entry(font='Arial 32')
    nickname_field.insert(0, nickname)
    nickname_field.configure(state='readonly')
    nickname_field.grid(row=5, column=2, columnspan=2, sticky=W + E, pady=10, padx=40)
    ...
    token_field = Entry(font='Arial 32')
    token_field.insert(0, account_hash)
    token_field.configure(state='readonly')
    token_field.grid(row=6, column=2, columnspan=2, sticky=W + E, pady=10, padx=40)
...
async def main(sending_port, sending_host):
    ...
    main_title = Label(root, text='Регистрация нового аккаунта', font='Arial 35', bd=20, bg='lightblue',
                       foreground='black')
    main_title.grid(row=0, column=0, columnspan=10, sticky=N)

    user_nickname = Label(root, text="Имя:", font='Arial 32', bd=20, bg='lightblue', foreground='black')
    user_nickname.grid(row=1, column=1, sticky=W, pady=10, padx=10)

    nickname_input_field = Entry(root, font='Arial 32', bg='white', fg='black')
    nickname_input_field.grid(row=1, column=2, columnspan=2, sticky=W + E, padx=40)

    send_button = Button(root, text="Отправить", width=15, height=-3, font='Arial 20',
...
if __name__ == '__main__':
    env.read_env()
    sending_port = env.int('SENDING_PORT')
